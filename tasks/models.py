...
class RecurringTaskTemplate(models.Model):
    ...
    reviewer = models.ForeignKey(Member, null=True, help_text="A reviewer that will be copied to instances of the recurring task.")
...
class Task(models.Model):
    ...
    work_estimate = models.IntegerField(default=0, help_text="Provide an estimate of how much work this tasks requires, in minutes. This is work time, not elapsed time.")
    reviewer = models.ForeignKey(Member, null=True, related_name="tasks_to_review", help_text="If required, a member who will review the completed work and either accept or reject it.")
    # Other fields:
    deadline = models.DateField(null=True, help_text="If appropriate, specify a deadline by which the task must be completed.")
